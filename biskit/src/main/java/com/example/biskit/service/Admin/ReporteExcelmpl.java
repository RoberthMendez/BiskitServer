...
public class ReporteExcelmpl implements ReporteExcelService{
    ...
    @Autowired private TratamientosService tratamientosService;
    ...
    public byte[] generarReporteUltimoMes() throws IOException {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            String nombreMes = ultimoMes.getMonth()
            ...
            // Título principal
            fila = crearTitulo(hoja1, estiloPrincipal, workbook,
                "REPORTE MENSUAL — VETERINARIA BISKIT", fila, 1);
            fila = crearTitulo(hoja1, estiloSubtitulo, workbook,
                "Período: " + periodoTexto, fila, 1);
            fila++; // fila vacía

            // Sección: Veterinarios
            fila = crearEncabezadoSeccion(hoja1, estiloEncabezado, "VETERINARIOS", fila, 1);
            fila = crearFila(hoja1, estiloDato, "Total de veterinarios",
                vetService.getVetsCount(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDatoAlt, "Veterinarios activos",
                vetService.getVetsActivosCount(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDato, "Veterinarios inactivos",
                vetService.getVetsInactivosCount(), estiloNumero, fila);
            fila++;

            // Sección: Mascotas
            fila = crearEncabezadoSeccion(hoja1, estiloEncabezado, "MASCOTAS", fila, 1);
            fila = crearFila(hoja1, estiloDato, "Total de mascotas",
                petsService.getPetsCount(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDatoAlt, "Mascotas activas",
                petsService.getMascotasActivasCount(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDato, "Mascotas inactivas",
                petsService.getPetsInactivosCount(), estiloNumero, fila);
            fila++;

            // Sección: Tratamientos y ventas
            fila = crearEncabezadoSeccion(hoja1, estiloEncabezado,
                "TRATAMIENTOS Y VENTAS — " + periodoTexto, fila, 1);
            fila = crearFila(hoja1, estiloDato, "Total de tratamientos",
                tratamientosService.countTratamientosUltimoMes(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDatoAlt, "Unidades de drogas vendidas",
                tratamientosService.getVentasTotalesMes(), estiloNumero, fila);
            fila = crearFila(hoja1, estiloDato, "Ganancias por drogas (COP)",
                tratamientosService.getGananciasTotalesMes(), estiloNumero, fila);
            ...
            fila2 = crearTitulo(hoja2, estiloPrincipal, workbook,
                "TOP 5 DROGAS MÁS VENDIDAS", fila2, 2);
            fila2 = crearTitulo(hoja2, estiloSubtitulo, workbook,
                "Período: " + periodoTexto, fila2, 2);
            fila2++;
            ...
            fila3 = crearTitulo(hoja3, estiloPrincipal, workbook,
                "TOP 5 ENFERMEDADES MÁS COMUNES", fila3, 2);
            fila3++;
            ...
            fila4 = crearTitulo(hoja4, estiloPrincipal, workbook,
                "TRATAMIENTOS POR DROGA", fila4, 1);
            fila4 = crearTitulo(hoja4, estiloSubtitulo, workbook,
                "Período: " + periodoTexto, fila4, 1);
            fila4++;
    ...
    private int crearTitulo(XSSFSheet hoja, XSSFCellStyle estilo,
            XSSFWorkbook wb, String texto, int fila, int lastCol) {
    ...
    private int crearEncabezadoSeccion(XSSFSheet hoja, XSSFCellStyle estilo,
            String texto, int fila, int lastCol) {
        Row row = hoja.createRow(fila);
    ...
    private int crearFila(XSSFSheet hoja, XSSFCellStyle estiloTexto,
            String etiqueta, Long valor, XSSFCellStyle estiloNum, int fila) {
        Row row = hoja.createRow(fila);
    ...
    private XSSFCellStyle crearEstiloDato(XSSFWorkbook wb) {
        XSSFCellStyle s = wb.createCellStyle();
        s.setFillForegroundColor(new XSSFColor(new byte[]{(byte)255, (byte)255, (byte)255}, null));
        s.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        s.setVerticalAlignment(VerticalAlignment.CENTER);
        setBorder(s);
        XSSFFont f = wb.createFont();
    ...
    private XSSFCellStyle crearEstiloDatoAlt(XSSFWorkbook wb) {
        XSSFCellStyle s = wb.createCellStyle();
        s.setFillForegroundColor(new XSSFColor(new byte[]{(byte)235, (byte)241, (byte)251}, null));
        s.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        s.setVerticalAlignment(VerticalAlignment.CENTER);
        setBorder(s);
        XSSFFont f = wb.createFont();
