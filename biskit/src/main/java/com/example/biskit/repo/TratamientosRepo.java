...
public interface TratamientosRepo extends JpaRepository<Tratamiento, Long> {
  ...
  @Query(
    "SELECT COUNT(t) FROM Tratamiento t WHERE YEAR(t.fecha) = :year AND MONTH(t.fecha) = :month"
  )
  ...
  @Query(
    "SELECT COUNT(t) FROM Tratamiento t JOIN t.drogas d WHERE d.id = :medicamentoId AND t.fecha >= :startDate"
  )
  Long getNumTratamientosDrogaDesde(
