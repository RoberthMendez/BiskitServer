...
public class DataLoader implements CommandLineRunner {
        ...
        public void cargarCredenciales(){
          ...
          for(Client client: clientsRepo.findAll()){
            Credenciales credenciales = Credenciales.builder().usuario(client.getCorreo()).password(client.getCedula()).build();
            credencialesRepo.save(credenciales);
            client.setCredenciales(credenciales); 
          }

          for(Vet vet: vetsRepo.findAll()){
            Credenciales credenciales = Credenciales.builder().usuario(vet.getCorreo()).password(vet.getCedula()).build();
            credencialesRepo.save(credenciales);
            vet.setCredenciales(credenciales); 
          }

          for(Admin admin: adminRepo.findAll()){
            Credenciales credenciales = Credenciales.builder().usuario(admin.getCorreo()).password(admin.getCedula()).build();
            credencialesRepo.save(credenciales);
        ...
        public void cargarTratamientos(){
          ...
          for(int i = 0; i < 10; i++){
            ...
            Tratamiento tratamiento = Tratamiento.builder().fecha(Date.valueOf(fecha)).build();
