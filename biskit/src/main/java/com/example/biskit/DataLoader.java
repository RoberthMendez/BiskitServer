...
public class DataLoader implements CommandLineRunner {
  ...
  public void cargarTratamientos() {
    ...
    for (int i = 0; i < 200; i++) {
      ...
      while (drogasTratamiento.size() < cantidadDrogasTratamiento) {
        ...
        if (
          droga != null && droga.getUnidadesDisponibles() > 0 && !drogasTratamiento.contains(droga)
        ) {
  ...
  private void cargarHorariosDia() {
    ...
    for (String diaSemana : diasSemana) {
      ...
      for (Turno turno : turnosDia) {
        ...
        for (Vet vet : vetsDisponibles) {
          ...
          boolean vetDisponible =
            diasAsignadosPorVet.get(vetId) < 5 && !diasPorVet.get(vetId).contains(diaSemana);
          boolean mejorOpcion =
    ...
    for (Vet vet : veterinarios) {
      ...
      while (diasAsignadosPorVet.get(vetId) < 5) {
        ...
        if (diasPorVet.get(vetId).contains(diaSemana)) continue;
  ...
  private boolean intentarCrearCitaBalanceada(
    ...
    List<Long> vetsOrdenados = horariosPorVet
      ...
      .stream()
      .sorted(
        java.util.Comparator.comparingInt((Long vetId) ->
          citasPorVet.getOrDefault(vetId, 0)
  ...
  private List<HorarioDia> ordenarHorariosDeSemana(
    ...
    return horarios
      .stream()
      .sorted(
        java.util.Comparator.comparingInt((HorarioDia horario) ->
          citasPorDia.getOrDefault(horario.getDiaSemana(), 0)
        )
