...
public class PetsRepoTest {
  ...
  public void setUp() {
    ...
    Especie perro = especiesRepo.save(Especie.builder().nombre("Perro").build());

    Especie gato = especiesRepo.save(Especie.builder().nombre("Gato").build());

    labrador = razaRepo.save(Raza.builder().nombre("Labrador").especie(perro).build());

    siames = razaRepo.save(Raza.builder().nombre("Siames").especie(gato).build());

    parvovirus = enfermedadRepo.save(Enfermedad.builder().nombre("Parvovirus").build());

    rinotraqueitis = enfermedadRepo.save(Enfermedad.builder().nombre("Rinotraqueitis").build());

    firulais = petsRepo.save(
      Pet.builder()
        .nombre("Firulais")
  ...
  public void PetsRepo_save_PetWithoutFechaNacimiento_ThrowsException() {
    ...
    Assertions.assertThatThrownBy(() -> petsRepo.save(pet)).isInstanceOf(Exception.class);
  }
  ...
  public void PetsRepo_findAll_NotEmptyList() {
    ...
    Assertions.assertThat(pets)
      .extracting(Pet::getNombre)
      .containsExactlyInAnyOrder("Firulais", "Michi", "Rex", "Luna");
    ...
    Assertions.assertThat(pets)
      .extracting(Pet::getNombre)
      .containsExactlyInAnyOrder("Firulais", "Michi", "Rex", "Luna", "Fido");
  }
